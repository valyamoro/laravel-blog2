    ...
    <select id="category_id" name="category_id" class="custom-select">
        @if($categories->isNotEmpty())
            @foreach($categories as $idx => $name)
                <option value="{{ $idx }}"
                        @if(isset($item) && $item->category->id === $idx) selected @endif>{{ $name }}</option>
            @endforeach
        @endif
    </select>
</div><br>
<div class="form-group">
    <label>Тэги</label>
    <select name="tags[]" class="select2 select2-hidden-accessible" multiple="" data-placeholder="Выбрать тэг" style="width: 100%;" data-select2-id="7" tabindex="-1" aria-hidden="true">
